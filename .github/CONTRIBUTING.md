...
2. Install the Go toolchain matching the `go` directive in `go.mod`
...
3. Write tests for your changes; core packages are expected to stay above 95% statement coverage.
...
6. Commit messages should follow the [Conventional
   ...
   - `fix(classifier): treat an empty title as authored`
...
10. Markdown style: When creating or editing Markdown files, aim for a
   ...
   long URLs are exceptions and may exceed the limit.

## Privacy Rules for Contributions

This is a public repository for a service that processes one athlete's GPS
data. Fixtures, tests and examples must use **synthetic coordinates and fake
identifiers** only. Never commit real start/end positions, athlete IDs, access
or refresh tokens, or reverse-geocoded points of interest.
