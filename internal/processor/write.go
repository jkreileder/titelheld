...
func (p *Processor) write(
	...
	p.recordRoute(ctx, athleteID, title, logger)
...
//
// Never fatal either: an uncounted ride costs a callback in a later title.
func (p *Processor) recordRoute(
	...
	if _, err := p.deps.Store.RecordRoute(
		ctx, athleteID, title.Fingerprint, p.deps.Now()); err != nil {
