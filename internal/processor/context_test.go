...
func TestHumanTitleIsRememberedAndNeverRenamed(t *testing.T) {
	...
	if entry, named, err := h.store.Named(t.Context(), 4242, humanID); err != nil || !named || entry.Title != humanTitle {
...
func TestHumanTitleIsRecordedInDryRun(t *testing.T) {
	...
	if entry, named, _ := h.store.Named(t.Context(), 4242, 777); !named || entry.Title != "Fünf auf einen Streich" {
