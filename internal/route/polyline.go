// Package route matches rides by the set of grid cells they traverse.
//
// A ride's summary polyline is decoded, every point is snapped to a grid, and
// the distinct cell SET is kept per named ride. Two rides are the same route
// when their sets' Jaccard similarity reaches a threshold. Sets are
// unordered, so matching is direction-blind by construction — an out-and-back
// is its own reverse, and a loop ridden either way round leaves the same
// cells.
//
// Everything here is pure: no store, no HTTP, no clock. The caller decides
// which rides enter a comparison window and what happens to a verdict.
package route
...
var ErrBadPolyline = errors.New("route: malformed polyline")
...
//
// Reimplemented here rather than imported from internal/geo so this package
// stays self-contained for the spike; the algorithm is identical, and the two
// are kept honest by the same worked example from Google's specification in
// the tests.
func DecodePolyline(encoded string) ([]Point, error) {
	...
	for index := 0; index < len(encoded); {
		...
		// A corrupt polyline that still decodes would otherwise be snapped to
		// cells in the wrong hemisphere and matched against rides that never
		// went there.
